...
收藏與瀏覽仍暫存本機；購物車仍為本機。雲端模式暫停結帳，以免使用舊本機結帳流程宣告下單成功。
展示模式原本的模擬結帳不變。商家上架頁仍是展示原型，尚未取得正式商家帳號或寫入 MySQL 的權限。

## 伺服器更新及驗證

本次不需要新的資料庫遷移；需已完成 schema.sql 及 001、002 的既有結構。
...
學校真實 MySQL 目錄驗收、外部 HTTPS 連線與真機畫面驗收仍待執行。
接下來需完成 App 會員活動 API 串接、正式商家管理與上架流程。詳見 [部署狀態](../docs/deployment_status.md)。
